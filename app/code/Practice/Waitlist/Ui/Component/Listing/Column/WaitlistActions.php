...
/**
 * WaitlistActions.php
 * This class builds the Action links (like Delete) for each row in the UI Component Grid.
 */
namespace Practice\Waitlist\Ui\Component\Listing\Column;
...
class WaitlistActions extends Column
{
    /**
     * @var UrlInterface
     ...
     * We loop through every row and attach the 'Delete' URL.
     *
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['entity_id'])) {
                    // Create the 'delete' action for this specific row
                    // $this->getData('name') gets the name of this column from the XML ("actions")
                    $item[$this->getData('name')] = [
                        'delete' => [
                            // Generate the secure URL: /admin/waitlist/index/delete/id/5/
                            'href' => $this->urlBuilder->getUrl(
                                'waitlist/index/delete',
                                ['id' => $item['entity_id']]
                            ),
                            'label' => __('Delete') // The text that appears in the dropdown
                        ]
                    ];
                }
            }
